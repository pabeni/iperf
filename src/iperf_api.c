    ...
    while ((flag = getopt_long(argc, argv, "ge:p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
		...
		if (!optarg) {
		    ...
		    return -1;
                ...
                test->settings->gso_size = atoi(optarg);
                break;
            case 'c':
                if (test->role == 's') {
                    i_errno = IESERVCLIENT;
...
send_parameters(struct iperf_test *test)
{
	...
	if (test->settings->gro)
	    cJSON_AddNumberToObject(j, "udp_gro", test->settings->gro);
	if (test->settings->gso_size)
	    cJSON_AddNumberToObject(j, "udp_gso_size", test->settings->gso_size);
...
get_parameters(struct iperf_test *test)
{
	...
	if ((j_p = cJSON_GetObjectItem(j, "udp_gso_size")) != NULL)
	    test->settings->gso_size = j_p->valueint;
